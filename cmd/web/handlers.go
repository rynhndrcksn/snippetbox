...
// snippetCreateForm represents the form used when making new snippets.
...
func (app *application) snippetCreatePost(w http.ResponseWriter, r *http.Request) {
	...
	// Validation checks so that they operate on the snippetCreateForm instance.
	...
	// Save the new snippet in the database.
	...
	// Add a flash message to the session manager
	app.sessionManager.Put(r.Context(), "flash", "Snippet successfully created!")

	http.Redirect(w, r, fmt.Sprintf("/snippet/view/%d", id), http.StatusSeeOther)
}

// userRegisterForm represents the form used when users register.
...
func (app *application) userRegisterPost(w http.ResponseWriter, r *http.Request) {
	...
	// Parse form data into our above userRegisterForm
	err := app.decodePostForm(r, &form)
	if err != nil {
		app.clientError(w, http.StatusBadRequest)
		return
	...
	form.CheckField(validator.MinChars(form.Password, 12), "password", "Password must be at least 12 characters long")
	form.CheckField(validator.MatchesStrings(form.Password, form.ConfirmPassword), "password", "Passwords don't match")
